...
> 来源: https://tencentarena.com/docs/p-competition-legged_robot_competition_26/22.0.12/guidebook/dev-guide/agent/
> 抓取时间: 2026-05-19
...
1. 评估时部分训练配置（如域随机化、观测噪声）默认关闭，以确保评估结果的一致性和可复现性。
2. `task` 必须与 `mode` 保持一致；若不一致，本轮评估结果没有意义（评分字段与地形语义错位）。
3. `num_envs` 配置小于等于 16 时会产出 mp4 视频用于可视化观察模型表现，大于 16 时将不会产出 mp4 视频。